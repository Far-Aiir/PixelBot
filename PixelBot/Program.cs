...
namespace PixelBot
{
    ...
    public class Start
    {
        ...
        static void Main()
        {
            ...
            using (StreamWriter file = File.CreateText(BotPath + "TokensTemplate" + ".json"))
            {
                ...
                serializer.Serialize(file, PixelBot.Tokens);
            ...
            {
                JsonSerializer serializer = new JsonSerializer();
                PixelBot.Tokens = (TokenClass)serializer.Deserialize(file, typeof(TokenClass));
            }
            new PixelBot().RunBot().GetAwaiter().GetResult();
        }
    }
    ...
    public class PixelBot
    {
        ...
        public static TokenClass Tokens = new TokenClass();
    ...
    public class Main : ModuleBase
    {
        ...
        public async Task Test(string Region = "", [Remainder] string User = "")
        {
            ...
            else
            {
                ...
                try
                {
                    IRiotClient RiotClient = new RiotClient(PixelBot.Tokens.Riot);
        ...
        {
            await Context.Channel.SendMessageAsync("<:vainglory:325848912470736896> | <:overwatch:325848873312714752> | <:xbox:325848933949505536> | <:wot:325848924290023436> | <:minecraft:325847826040881163> | <:pokemon:325848881768169472> | <:steam:325848894678368266> | <:osu:325848862130569217>");
            return;
            switch (all)
            {
                case "all":
    ...
    public class Game : ModuleBase
    {
        ...
        {
            await ReplyAsync("`Currently broken`").ConfigureAwait(false);
            return;
        ...
        public async Task Xboxuser(string User)
        {
            ...
            try
            {
                ...
                GetUserId.Headers.Add("X-AUTH", PixelBot.Tokens.Xbox);
                ...
                OnlineHttp.Headers.Add("X-AUTH", PixelBot.Tokens.Xbox);
                ...
                HttpUserGamercard.Headers.Add("X-AUTH", PixelBot.Tokens.Xbox);
                ...
                HttpUserFrineds.Headers.Add("X-AUTH", PixelBot.Tokens.Xbox);
        ...
        public class SteamComs : ModuleBase
        {
            ...
            public async Task Steamu([Remainder] string User = null)
            {
                ...
                SteamWebAPI.SetGlobalKey(PixelBot.Tokens.Steam);
        ...
        public async Task Osu(string User = null)
        {
            ...
            try
            {
                HttpWebRequest GetUserId = (HttpWebRequest)WebRequest.Create("https://osu.ppy.sh/api/get_user?k=" + PixelBot.Tokens.Osu + "&u=" + User);
                GetUserId.Method = WebRequestMethods.Http.Get;
                GetUserId.Accept = "application/json";
                HttpWebResponse response = (HttpWebResponse)GetUserId.GetResponse();
    ...
    public class Media : ModuleBase
    {
        ...
        public async Task TwitchSearch(string Search = null)
        {
            ...
            var client = new TwitchAuthenticatedClient(PixelBot.Tokens.Twitch, PixelBot.Tokens.TwitchAuth);
        ...
        public async Task TwitchNotify(string Option = null, string Channel = null)
        {
            ...
            if (Option == "me")
            {
                ...
                JsonSerializer serializer = new JsonSerializer();
                using (StreamWriter file = File.CreateText(PixelBot.BotPath + $"Twitch\\user-{Context.User.Id}-{Channel.ToLower()}.json"))
                {
            ...
            else if (Option == "here" || Option == "channel")
            {
                ...
                JsonSerializer serializer = new JsonSerializer();
                using (StreamWriter file = File.CreateText(PixelBot.BotPath + $"Twitch\\channel-{Context.Guild.Id.ToString()}-{Context.Channel.Id}-{Channel.ToLower()}.json"))
                {
                    serializer.Serialize(file, NewNotif);
                }
                MyServices.TwitchNotifications.Add(NewNotif);
    ...
    public class Help : ModuleBase
    {
        ...
        public async Task Pag(string Option = "")
        {
            ...
            foreach (var CMD in PixelBot._commands.Commands.Where(x => x.Module.Name == "Misc"))
            {
                MiscList.Add($"[ p/{CMD.Remarks} ][ {CMD.Summary} ]");
            }
            foreach (var CMD in PixelBot._commands.Commands.Where(x => x.Module.Name == "Game"))
            {
                GameList.Add($"[ p/{CMD.Remarks} ][ {CMD.Summary} ]");
            }
            foreach (var CMD in PixelBot._commands.Commands.Where(x => x.Module.Name == "Media"))
            {
                MediaList.Add($"[ p/{CMD.Remarks} ][ {CMD.Summary} ]");
            }
            foreach (var CMD in PixelBot._commands.Commands.Where(x => x.Module.Name == "prune"))
            {
            ...
            "```md" + Environment.NewLine + "< ◄ Games |     Prune | >" + Environment.NewLine + PruneText + "```"
                ...
                await DiscordUtils.SendPaginator(pages, "Commands List", Context, embed).ConfigureAwait(false);
            }
        }
